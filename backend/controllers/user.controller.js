...
export const followOrUnfollow = async (req,res) =>{
    try {
        ...
        const followkarunga = req.params.id
        if(followkarnewala===followkarunga){
            return res.status(400).json({
        ...
        const targetUser = await User.findById(followkarunga)
        ...
        const isFollowing = user.following.includes(followkarunga)
        if(isFollowing){
            //unfollow logic
            await Promise.all([
                User.updateOne({_id:followkarnewala},{$pull:{following:followkarunga}}),
            ...
            //follow logic
            await Promise.all([
                User.updateOne({_id:followkarnewala},{$push:{following:followkarunga}}),
